...
def bench_wrapper(args, inputs):
    ...
    wrapper = flashinfer.BatchDecodeWithPagedKVCacheWrapper(workspace, "NHD", use_tensor_cores=False)
    wrapper.plan(
        inputs["kv_indptr"],
        inputs["kv_indices"],
...
def main():
    ...
    try:
        ...
        print(f"[skip trtllm: {type(e).__name__}: {str(e)[:120]}]")
        ms_trt = None
    try:
        ms_trt_conv = bench_trtllm_with_conv(args, inputs)
    except Exception as e:
        print(f"[skip trtllm+csr_conv: {type(e).__name__}]")
        ms_trt_conv = None
    try:
